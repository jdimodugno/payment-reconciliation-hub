...
- **Dead-letter (dos superficies, hoy):**
  ...
  - **Retriable agotado** → por ahora cae en el **failed set de BullMQ** (Redis, volátil) — **todavía NO** aterriza en el anexo durable. Gap consciente (ver ADR-011, scope). **Decisión (debe entrar, no es opcional):** por invariante todo evento terminal conserva el contexto de su última ejecución, así que el agotamiento *debe* persistirse al anexo. Mecanismo: hook `@OnWorkerEvent('failed')` con guarda de agotamiento (`attemptsMade >= attempts`) + **sweep de reconciliación** (failed-en-BullMQ-sin-fila-en-anexo → repara), porque las ventanas de falla (worker crashea / hook no corre / append falla) **convergen al mismo observable** y se cubren con un detector del síntoma, no protegiendo cada paso. **Diferido del MVP** con razón: el append idempotente choca con el diseño append-only multi-fila (audit trail) → reconciliar ambas intenciones amerita diseño planificado. **Consecuencia para la demo:** el camino DLQ demostrable hoy es el **no-retriable**; una falla retriable-agotada todavía no aparece en el anexo.
...
| `GET /webhooks/pipeline-status` | Read-only observability lens over the **webhook pipeline backlog** (it reconciles nothing — no provider side, no comparison): unprocessed events (`received` / `pending_manual_review` with `processed_at IS NULL`), ordered by `received_at` ascending so aging is visible, plus terminal-status counts (`processed` / `pending_manual_review`) and the number of dead-lettered events. Does **not** assert orphan-hood yet — no state machine / processing-window modeled. A failed read answers `500`: "could not read" is unexpected infrastructure failure, not "read zero", and a `200` carrying an error body would be counted as success by monitoring. |
...
| `POST /reconciliation/runs` | Triggers a reconciliation batch (ADR-017): reads the internal side (transactions) and the provider side (re-derived from received events), crosses them in a pure matcher, and persists the discrepancies found under a `runId`. Answers `201` with `{ runId, scanned: { internal, provider, unreadable }, discrepancies, byKind }` — `scanned` travels explicitly because zero findings over zero data and zero findings over thousands of pairs read the same otherwise. `POST` and not `GET` because a run **writes**; the resource is the run, not the status. **Not idempotent**: every call creates a new run. |
